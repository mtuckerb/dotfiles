...
let light_theme = {
    ...
    separator: dark_gray
    leading_trailing_space_bg: { attr: n } # no fg, no bg, attr none effectively turns this off
    header: {fg: '#0096FF' bg: '#EDF6F9' attr: b}
    empty: blue
    # Closures can be used to choose colors for specific values.
    # The value (in this case, a bool) is piped into the closure.
...
ssh-agent
ssh-add ~/.ssh/github
ssh-add ~/.ssh/id_farkwar
alias node_cli = node -i -e (cat ./cli.js)
ssh-add ~/.ssh/aws-tucker

def git_pull_all [] {
   ls | each {|file|
   print "-------------------------------"
    print $"(ansi purple_bold) ($file.name) (ansi reset)"
    cd $"($file.name)"
    if (".git" |path exists) {
      git pull
      }
